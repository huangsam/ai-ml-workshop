      ...
      <nav className="flex-1 overflow-y-auto py-4 custom-scrollbar">
        {Object.entries(groups).map(([module, moduleTasks]) => (
          <div key={module} className="mb-2">
            <p className="px-5 mb-2 text-xs font-semibold uppercase tracking-widest text-gray-400">
            ...
            {moduleTasks.map((t) => {
              ...
              return (
                <button
                  key={`${t.module}/${t.task}`}
                  onClick={() => onSelect(t)}
                  className={`relative w-full text-left px-5 py-2.5 text-sm transition-all duration-300 group ${
                ...
                >
                  {isActive && (
                    <>
                      ...
                      <div className="absolute left-0 top-1/2 -translate-y-1/2 w-1 h-8 bg-gradient-to-b from-indigo-500 to-purple-600 rounded-r shadow-[0_0_10px_rgba(99,102,241,0.5)]" />
                    </>
                  )}
                  <span
                    className={`${
                      isActive ? "font-medium" : "font-normal"
                    } transition-all duration-300 group-hover:translate-x-0.5 inline-block`}
                  >
